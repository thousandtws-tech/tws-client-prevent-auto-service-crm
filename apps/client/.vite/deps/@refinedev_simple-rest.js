...
var generateFilter = (filters) => {
  ...
  if (filters) {
    filters.map((filter2) => {
      if (filter2.operator === "or" || filter2.operator === "and") {
        throw new Error(
          `[@refinedev/simple-rest]: \`operator: ${filter2.operator}\` is not supported. You can create custom data provider. https://refine.dev/docs/api-reference/core/providers/data-provider/#creating-a-data-provider`
        );
      }
      if ("field" in filter2) {
        const { field, operator, value } = filter2;
        if (field === "q") {
...
var dataProvider = (apiUrl, httpClient = axiosInstance) => ({
  getList: async ({ resource, pagination, filters, sorters, meta }) => {
    ...
    const urlWithQuery = Object.keys(combinedQuery).length ? `${url}?${(0, import_query_string2.stringify)(combinedQuery)}` : url;
  ...
  getMany: async ({ resource, ids, meta }) => {
    ...
    const { data } = await httpClient[requestMethod](
      `${apiUrl}/${resource}?${(0, import_query_string2.stringify)({ id: ids })}`,
      { headers }
    );
    return {
      data
  ...
  custom: async ({
    ...
    if (sorters) {
      ...
      if (generatedSort) {
        ...
        requestUrl = `${requestUrl}&${(0, import_query_string2.stringify)(sortQuery)}`;
      }
    }
    if (filters) {
      const filterQuery = generateFilter(filters);
      requestUrl = `${requestUrl}&${(0, import_query_string2.stringify)(filterQuery)}`;
    }
    if (query) {
      requestUrl = `${requestUrl}&${(0, import_query_string2.stringify)(query)}`;
    }
    let axiosResponse;
    switch (method) {
      case "put":
